...
# less than or equal to the given limit `t`, if that sum exists, or 
# otherwise nil, null, None, Nothing, depending on the language.
...
def choose_best_sum(t, k, ls)
  ls.combination(k).map{ |distance| distance.inject(:+) }.select{ |sum| sum <= t }.max
end

puts choose_best_sum(174, 3, [50, 55, 57, 58, 60])

ts = [50, 55, 56, 57, 58]
puts choose_best_sum(163, 3, ts) # -> 163

ys = [91, 74, 73, 85, 73, 81, 87] 
puts choose_best_sum(230, 3, ys) # -> 228
